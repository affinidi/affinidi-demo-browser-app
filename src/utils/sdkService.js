...
class SdkService {
  ...
  async getSharableCredentialLink() {
    const shareLinkParam = {
      ttl:"10"
    }
    const { credentials } = await this.getDidAndCredentials()
    const response = await cloudWalletApi.post(`/wallet/credentials/${credentials[0].id}/share`, shareLinkParam)
